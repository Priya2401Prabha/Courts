...
class ProductTemplate(models.Model):
    ...
    os_po_qty = fields.Float(
        string="O/S Purchased Units",
    ...
    weekly_valuation = fields.Float(
        string="Weekly Valuation",
        compute="_compute_weekly_valuation",
        store=True
    )

    weekly_quantity = fields.Float(
        string="Weekly Quantity",
        compute="_compute_weekly_valuation",
        store=True
    )

    weekly_valuation_new = fields.Float(
        string="Weekly Valuation",
        compute="_compute_weekly_valuation",
        store=True
    )

    # default_conversion_uom_id = fields.Many2one('uom.uom', string="Default Conversion UoM")
    ...
    def update_compute_sales(self):
        ...
        for product in all_products:
            ...
            product._compute_weekly_valuation(from_date, to_date)
            product._compute_os_po_value()
            # product._calculate_weekly_metrics(from_date, to_date)
    ...
    @api.depends('purchased_product_qty', 'qty_available')
    def _compute_os_po_qty(self):
        for rec in self:
            rec.os_po_qty = rec.purchased_product_qty - rec.qty_available

    @api.depends('product_variant_ids.purchase_order_line_ids.product_uom_qty',
                 'product_variant_ids.purchase_order_line_ids.qty_received',
                 'product_variant_ids.purchase_order_line_ids.price_unit',
                 'product_variant_ids.purchase_order_line_ids.state')
    def _compute_os_po_value(self):
        for template in self:
            # Collect all PO lines for all variants
            po_lines = template.product_variant_ids.mapped('purchase_order_line_ids') \
                .filtered(lambda l: l.state in ['purchase', 'done'])

            # Compute total outstanding value
            total = sum(max(0, l.product_uom_qty - l.qty_received) * l.price_unit for l in po_lines)
            template.os_po_value = total
    ...
    @api.depends('product_variant_ids.stock_move_ids.move_line_ids.valuation_value',
                 'product_variant_ids.stock_move_ids.move_line_ids.quantity',
                 'product_variant_ids.stock_move_ids.move_line_ids.location_id',
                 'product_variant_ids.stock_move_ids.move_line_ids.location_dest_id',
                 'product_variant_ids.stock_move_ids.move_line_ids.is_usage_location',
                 'product_variant_ids.stock_move_ids.move_line_ids.date',
                 'product_variant_ids.stock_move_ids.move_line_ids.state','from_date', 'to_date', 'standard_price')
    def _compute_weekly_valuation(self, from_date=None, to_date=None):
        for template in self:
            today = fields.Datetime.now()

            # Use passed dates if available, else fallback to template fields or defaults
            calc_from = from_date or template.from_date or (today - timedelta(days=7))
            calc_to = to_date or template.to_date or today

            # Ensure datetime type
            if isinstance(calc_from, fields.Date):
                calc_from = fields.Datetime.from_string(str(calc_from))
            if isinstance(calc_to, fields.Date):
                calc_to = fields.Datetime.from_string(str(calc_to))

            move_lines = template.product_variant_ids.mapped(
                'stock_move_ids.move_line_ids'
            ).filtered(
                lambda l: (
                        l.state == 'done'
                        and calc_from <= l.date <= calc_to
                        and l.is_usage_location
                        and l.location_id != l.location_dest_id
                )
            )

            template.weekly_valuation = sum(move_lines.mapped('valuation_value'))
            template.weekly_quantity = sum(move_lines.mapped('quantity'))
            template.weekly_valuation_new = template.weekly_quantity * template.standard_price

    # @api.depends('product_variant_ids.stock_move_ids.move_line_ids.valuation_value',
    #              'product_variant_ids.stock_move_ids.move_line_ids.quantity',
    #              'product_variant_ids.stock_move_ids.move_line_ids.date',
    #              'product_variant_ids.stock_move_ids.move_line_ids.state','from_date', 'to_date')
    # def _compute_weekly_valuation(self):
    #     for template in self:
    #         today = fields.Datetime.now()
    #
    #         to_date = template.to_date or today
    #         from_date = template.from_date or (today - timedelta(days=7))
    #         move_lines = template.product_variant_ids.mapped(
    #             'stock_move_ids.move_line_ids'
    #         ).filtered(
    #             lambda l: l.state == 'done' and
    #                       (not from_date or l.date >= from_date) and
    #                       (not to_date or l.date <= to_date)
    #         )
    #
    #         # Compute total outstanding value
    #         total = sum(move_lines.mapped('valuation_value'))
    #         total_qty = sum(move_lines.mapped('quantity'))
    #         template.weekly_valuation = total
    #         template.weekly_quantity = total_qty
